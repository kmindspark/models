...
def draw_bounding_box_on_image(image,
  ...
  # Each display_str has a top and bottom margin of 0.01x.
...
def visualize_boxes_and_labels_on_image_array(
  ...
  for i in range(boxes.shape[0]):
    if max_boxes_to_draw == len(box_to_color_map):
      break
    if scores is None or scores[i] > 0.01:
...
class EvalMetricOpsVisualization(six.with_metaclass(abc.ABCMeta, object)):
  ...
  def __init__(self,
    ...
    self._min_score_thresh = 0.01#min_score_thresh
