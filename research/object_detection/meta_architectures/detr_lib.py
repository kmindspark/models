...
class Transformer(tf.keras.Model):
  ...
  def encode(self, encoder_inputs, training):
    ...
    with tf.name_scope("encode"):
      ...
      encoder_inputs = tf.keras.layers.Dropout(self._layer_postprocess_dropout, training=training)(encoder_inputs)
      #if training:
...
class PrePostProcessingWrapper(tf.keras.layers.Layer):
  ...
  def call(self, x, *args, **kwargs):
    ...
    y = tf.keras.layers.Dropout(self_postprocess_dropout, training=training)(y)
    #if training:
...
class Attention(tf.keras.layers.Layer):
  ...
  def build(self, input_shape):
    ...
    attention_initializer = _glorot_initializer(input_shape.as_list()[-1],
                                                self.hidden_size)
    self.query_dense_layer = layers.DenseEinsum(#tf.keras.layers.experimental.EinsumDense(
        #equation="abc,cde->abde",
        output_shape=(self.num_heads, size_per_head),
        kernel_initializer=attention_initializer,
        use_bias=False,
        name="query")
    self.key_dense_layer = layers.DenseEinsum(#tf.keras.layers.experimental.EinsumDense(
        #equation="abc,cde->abde",
        output_shape=(self.num_heads, size_per_head),
        kernel_initializer=attention_initializer,
        use_bias=False,
        name="key")
    self.value_dense_layer = layers.DenseEinsum(#tf.keras.layers.experimental.EinsumDense(
        #equation="abc,cde->abde",
        output_shape=(self.num_heads, size_per_head),
        kernel_initializer=attention_initializer,
        use_bias=False,
    ...
    self.output_dense_layer = layers.DenseEinsum(#tf.keras.layers.experimental.EinsumDense(
  ...
  def call(self, query_input, key_input, value_input, training, cache=None,
    ...
    weights = tf.keras.layers.Dropout(self.attention_dropout, training=training)(weights)
    #if training:
...
class FeedForwardNetwork(tf.keras.layers.Layer):
  ...
  def call(self, x, training):
    ...
    output = tf.keras.layers.Dropout(self._layer_postprocess_dropout, training=training)(output)
    #if training:
