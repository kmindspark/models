...
class DETRMetaArch(model.DetectionModel):
  def __init__(self,
    ...
    self.queries = tf.keras.backend.variable(tf.zeros([self.num_queries, self.hidden_dimension])) #tf.zeros([self.num_queries, self.hidden_dimension]), dtype=tf.float32) #tf.random.uniform([self.num_queries, self.hidden_dimension]) tf.Variable(initial_value=tf.zeros((self.num_queries, self.hidden_dimension)), trainable=True)
